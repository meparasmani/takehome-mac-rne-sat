...
The grading checks are cycle-exact: keep the readout path to a **single**
register stage from the sampled controls (update `res` / `res_valid` /
`ovf` from the same edge that samples `rd`). Extra pipeline stages on the
readout path will fail timing checks even if the numeric rounding is
correct. Between readouts, `res` **holds** its last value; it does not
clear when `res_valid` is low. Back-to-back `rd` cycles are permitted and
each takes its own snapshot.

**Rounding — round-half-to-even at the 8 LSBs.** Rounding applies only to
the readout snapshot (not during accumulation). For hardware, an
equivalent formulation that works for negatives is:

- `q = snapshot >>> 8` (arithmetic shift = `floor(snapshot / 256)`);
- `r = snapshot[7:0]` (non-negative remainder, `0 ≤ r ≤ 255`).
...
Avoid truncating-toward-zero division when forming `q`/`r` for negative
snapshots.
...
saturation applies to the **rounded** value. Exact rounded `-32768` is in
range and must not assert `ovf`.
